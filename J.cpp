...
int segtree5[N << 2];
int pt[N << 2];
int pt5[N << 2];void build(int node , int u , int v) {
if(u > v) return;
if(u == v) {
segtree[node] = val[u];
...
void update(int node , int u , int v, int idx , int val) {if(u > v | u > idx | v < idx) return;
if(u >= idx and v <= idx) {
segtree[node] = val;
return;
}
...
sort(twos.begin() , twos.end());assert(twos.size() >= 2);
